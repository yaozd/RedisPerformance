...
namespace Beetle.Redis
{
    ...
    public abstract class  RedisKey
    {
         ...
         * */
        /// <summary>
        /// 递增1--原子性操作--Redis所有单个命令的执行都是原子性的，这与它的单线程机制有关；
        /// </summary>
        /// <param name="db"></param>
        /// <returns></returns>
        public long Incr(RedisClient db = null)
        {
            db = RedisClient.GetClient(db);
            return db.INCR((string)DataKey);
        }
        /// <summary>
        /// 递减1--原子性操作--Redis所有单个命令的执行都是原子性的，这与它的单线程机制有关；
        /// </summary>
        /// <param name="db"></param>
        /// <returns></returns>
        public long Decr(RedisClient db = null)
        {
            db = RedisClient.GetClient(db);
